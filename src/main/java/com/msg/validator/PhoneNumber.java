...
public @interface PhoneNumber {
	String message() default Submit.PHONE_NUMBER_COULD_NOT_BE_EMPTY;
    Class<?> [] groups() default {};
    Class<? extends Payload> [] payload() default {};
    boolean emptyable() default false;
    int max() default 50;
    int min() default 1;
    String lengthmsg() default Submit.PHONE_NUMBER_LENGTH_ERROR;
    String regexp() default "((\\d{11})|^((\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})|(\\d{4}|\\d{3})-(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1})|(\\d{7,8})-(\\d{4}|\\d{3}|\\d{2}|\\d{1}))$)";
    String regexpmsg() default Submit.PHONE_NUMBER_COMPOSE_ERROR;
}
